...
def gcode(job):
    ...
    #code.extend(settings.to_gcode()) # too many lines. some gcode sender get stuck after a certain number of comment lines
...
def write_cutout(job):
    ...
    depth = settings.get('cut_depth')

    dp, dpfeed = get_nr_of_decimals(job)

    coords_out = job.cut_coords
    ...
    if settings.get('no_comments') is False:
        ...
        code.append('( cutting out the workpiece. )')

        code.append('( The tool paths were calculated based )')
        code.append('( on using a straight endmill bit with a )')
        code.append('( diameter of %.4f %s)' % (settings.get('cut_dia'), settings.get('units')))
        code.append('( with depth per cut of %.4f %s)' % (settings.get('cut_dpc'), settings.get('units')))
        code.append('( and a total depth of %.4f %s)' % (settings.get('cut_depth'), settings.get('units')))
    ...
    while go:
        z1 = z1 - dpc
        if z1 < -depth or isclose(z1, -depth, rel_tol=1e-5):
            # last pass
            go = False
            z1 = -depth
        print()
        ...
        if len(coords_out) > 0:
            ...
            while v_index < len(coords_out) - 1:
                ...
                if loop != loop_old:
                    ...
                    if x0 is None or y0 is None or (not isclose(x0, x1, rel_tol=1e-5)) or (not isclose(y0, y1, rel_tol=1e-5)):
                        # lift engraver
                        print(x0, x1, y0, y1)
                        code.append("G0 Z%s" % safe_val)
...
def write_clean_up(job, bit_type="straight"):
    ...
    # Cleanup
    maxDZ = settings.get('v_max_cut')
    rough_stock = 0 #settings.get('v_rough_stk') # never make a finishing pass for any clean up, wasted time in my opion
    zmin = 0.0
    roughing = True
    rough_again = False
    if rough_stock > 0:
